...
class sub_convert():
    ...
    def convert(raw_input, input_type='url', output_type='url', custom_set={'dup_rm_enabled': True, 'format_name_enabled': False}): # {'input_type': ['url', 'content'],'output_type': ['url', 'YAML', 'Base64']}
    ...
    def transfer(sub_content): # 将 URL 内容转换为 YAML 格式
        if '</b>' not in sub_content:
            ...
            print('订阅内容解析错误')
            return '订阅内容解析错误'
    def format(sub_content, output=True): # 对节点 Url 进行格式化处理, 输出节点的字典格式, output 为真时输出 YAML 文本
    ...
    def makeup(input, dup_rm_enabled=True, format_name_enabled=False): # 对节点进行区域的筛选和重命名，输出 YAML 文本 
