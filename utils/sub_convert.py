            ...
            for index in range(len(proxies_list)): # 不同节点订阅链接内容 https://github.com/hoochanlon/fq-book/blob/master/docs/append/srvurl.md
                ...
                if proxy['type'] == 'vmess': # Vmess 节点提取, 由 Vmess 所有参数 dump JSON 后 base64 得来。
                    ...
                    vmess_value = {
                        ...
                        'scy': proxy_config['cipher'], 'net': proxy_config['network'], 'type': None, 'host':  json.loads(proxy_config['ws-opts']['headers']['Host']),
                        'path':  json.loads(proxy_config['ws-opts']['path']), 'tls': proxy_config['tls'], 'sni': proxy_config['sni']
                        }
